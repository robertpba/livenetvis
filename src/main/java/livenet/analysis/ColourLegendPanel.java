...
public class ColourLegendPanel extends JPanel
{
    ...
    int maxValue;         // maximum histogram value
    int maxKey;           // maximum histogram key
    ...
    public ColourLegendPanel(Histogram hist, Color[] colours,
	...
	if (leftMargin == 0)
	    leftMargin = xMargin + markLength + labelGap +
		fm1.stringWidth(String.valueOf(maxValue)) + labelGap +
		fm2.getHeight();

	if (rightMargin == 0)
	    rightMargin = xMargin + markLength + labelGap +
		fm1.stringWidth(String.valueOf(maxKey)) + labelGap +
		fm2.getHeight();
	...
	if (maxValue > numYTicks)
	    increment = Math.round((float) (maxValue / numYTicks));
	...
	increment = 1;
	if (maxKey > numYTicks)
	    increment = Math.round((float) (maxKey / numYTicks));

	for (int i = 0; i <= maxKey; i += increment) {
	...
	int firstKey = hist.getMinKey();

	// label for first value (0) at bottom of legend
	String firstLabel = String.valueOf(0);
	...
	String lastLabel = String.valueOf(maxKey);
	...
	int nextKeyTick = (int) (maxKey / numXTicks);
	...
	for (int i = 0; i < colours.length; i++) {
	    ...
	    // Step 2: draw the polyline indicating number of workspaces
	    int histValue = hist.getValueAt(i);
	    if (histValue < 0)
		histValue = 0;
	    ...
	    // Step 3: draw the polyline indicating workspace density levels
	    int histKey = hist.getKeyAt(i);
		...
		drawnKeyTicks++;
		nextKeyTick =
		    (int) (((drawnKeyTicks +1) * maxKey) / numXTicks);
		if (DEBUG)
